...
if (Is-ValidPort $webPortRaw) {
  ...
  # Note: We do NOT set $env:PORT here because that would affect ALL child processes (including API).
  # Instead, PORT is set specifically when starting the web server below.
}
else {
  Write-Warning "WEB_BASE does not contain a valid port. WEB_PORT will not be set. (WEB_BASE='$env:WEB_BASE')"
}
