...
summary: How the project wires Cucumber-style BDD with Playwright for end-to-end tests.
---

This document describes the recommended wiring for a BDD stack using Gherkin (Cucumber-style) feature files executed with Playwright as the browser automation layer.

Key points:

- Feature files live under `docs/Stories/features/` (or `tests/e2e/features/`) and are small, focused, and executable.
- Use a lightweight runner that maps Gherkin steps to Playwright helper functions.
- Keep step definitions thin: delegate most logic to page objects / test helpers to keep steps readable and maintainable.
- Use tags to control runs (e.g., `@smoke`, `@regression`, `@unstable`).

Implementation notes:

- Prefer Playwright Test for orchestration and use a small adapter to consume Gherkin files. This keeps Playwright's parallelism, reporters, and fixtures.
- Use deterministic seeding for test data (see `5-test-data-and-seeding-deterministic.md`).
- Store BDD artifacts (traces, videos, screenshots) in `playwright-report/` and publish them from CI when failing.

Examples and snippets live in the repository's `scripts/` folder for wiring helpers.

Best practices:

- Keep feature files at the acceptance level (business intent), not implementation details.
- Use parameterized steps to avoid explosion of step definitions.
- Tag flaky tests with `@flaky` and add retries via Playwright config.

# Story 1 — Wire Up BDD Stack (Cucumber.js + Playwright + TypeScript)

**Objective**  
Create a reproducible BDD test stack where `cucumber-js` executes TypeScript step definitions that drive **Playwright** browser automation. One command runs locally; one command runs in CI. Produce minimal scaffolding (world, hooks, sample feature, page objects).
