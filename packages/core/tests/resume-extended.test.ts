...
// Mock generator that returns structured data (ResumeData) instead of a string
const mockStructuredDataGenerator = {
...
describe('ResumeService', () => {
  ...
  test('should throw an error when generator returns structured data', async () => {
    ...
    // Act & Assert
    await expect(resumeService.createResume(mockResumeData, mockJobDetails)).rejects.toThrow(
      'ResumeService expects generator to return a string. Received structured data.'
    );
    expect(mockStructuredDataGenerator.generateResume).toHaveBeenCalledWith(
      mockResumeData,
      mockJobDetails
    );
  });
